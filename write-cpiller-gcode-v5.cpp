...
vector<vector<vector<point> > >modfill; //完成的model填充线，一维层数，二维轮廓，三维该轮廓的变化线 线段坐标
...
//*********************将最内圈的点赋给shell1***********************************
   ...
   /*	for (int i = 0; i < shell1.size(); i++)                         //打印轮廓点坐标和偏移后外壳坐标
	{
		for (int j = 0; j < shell1[i].size(); j++)
		{
			...
			{
				cout << "times : " << k;
				for (int m = 0; m < shell[i][j][k].size(); m++)
				{				  
                      cout << "(" << shell[i][j][k][m].x << "," << shell[i][j][k][m].y << ")";
				}		
				cout << endl;
			}
			cout << endl;
...
void intersection()           //在这里面调节要不要变化
{
	for (int h = 0; h < shell1.size(); h++)
	{
		for (int i = 0; i < shell1[h].size(); i++)
		{
			...
			triangle(h); //产生第i层的变化线存在tripoint        0：不变三角，48不变六边形， i变化线

			polypoint = shell1[h][i];//把第i层的轮廓线给polypoint
			...
			modelfill.push_back(new_tripoint);              //存该轮廓的点
			new_tripoint.clear();
			
		}
		printf("%.2lf%%\r", h * 100.0 / model.size());
		modfill.push_back(modelfill);
		modelfill.clear();
	}
}
...
void writegcode_ultimaker3(float z) //cura
{
	...
	for (int i = 0; i < model.size() - 4; i++) //i代表层数，要加几层打实在的，界限就减几，再改高度   
	{
		...
		fprintf(fp, "M73 P%.f\n", float(  (100*i)/model.size() ) );
		for (int j = 0; j < shell1[i].size(); j++)
			//轮廓线
		{
			//打印最内层
			fprintf(fp, "G0 F2000 X%.3f Y%.3f Z%.1f\n", shell1[i][j][0].x, shell1[i][j][0].y, 0.3 + i * z);//3层高度加i*z，i从0开始

			fprintf(fp, "G1 F1200 X%.3f Y%.3f E%.5f\n", shell1[i][j][1].x, shell1[i][j][1].y, E += distance(shell1[i][j][0], shell1[i][j][1]) * t);

			for (int k = 2; k < shell1[i][j].size(); k++)

			{
					fprintf(fp, "G1 X%.3f Y%.3f E%.5f\n", shell1[i][j][k].x, shell1[i][j][k].y, E += distance(shell1[i][j][k-1], shell1[i][j][k]) * t);
			...
			for (int k = 0; k < shell[i][j].size(); k++)
			{
				fprintf(fp, "G0 F2000 X%.3f Y%.3f Z%.1f\n", shell[i][j][k][0].x, shell[i][j][k][0].y, 0.3 + i * z);//3层高度加i*z，i从0开始

				fprintf(fp, "G1 F1200 X%.3f Y%.3f E%.5f\n", shell[i][j][k][1].x, shell[i][j][k][1].y, E += distance(shell[i][j][k][0], shell[i][j][k][1]) * t);

				for (int m = 2; m < shell[i][j][k].size(); m++)

				{
					fprintf(fp, "G1 X%.3f Y%.3f E%.5f\n", shell[i][j][k][m].x, shell[i][j][k][m].y, E += distance(shell[i][j][k][m - 1], shell[i][j][k][m]) * t);
		...
		for(int j=0;j<modfill[i].size();j++)
		{ 		
			for (int k = 1; k < modfill[i][j].size(); k += 2)
			{
			fprintf(fp, "G0 F2000 X%.3f Y%.3f Z%.1f\n", modfill[i][j][k - 1].x, modfill[i][j][k - 1].y,0.3+i*z);
			fprintf(fp, "G1 F1200 X%.3f Y%.3f Z%.1f E%.5f\n", modfill[i][j][k].x, modfill[i][j][k].y, 0.3 + i * z, E += distance(modfill[i][j][k - 1], modfill[i][j][k]) * t);
			}
		}
...
void writegcode_makerbot(float z)//还是用cura的格式然后用x3g转换吧
{
	...
	fprintf(fp, "G1 X%f Y%f Z%f F3000\n", Bmin_x - 0.01, Bmin_y - 0.01, 0.27);
	//for (int i = 0; Bmin_x+i*4<Bmax_x; i++)  //底座
	//{
	...
	//fprintf(fp, ";LAYER:0\n");
	for (int i = 0; Bmin_x + 2 * i * 1 + 1 < Bmax_x; i++)  //第1、2层打实，第一层高度是0.27
	{
		fprintf(fp, "G1 X%f Y%f Z%f F1200\n", Bmin_x + 2 * i * 1, Bmin_y, 0.27);
		fprintf(fp, "G1 X%f Y%f Z%f F1200 A%f\n", Bmin_x + 2 * i * 1, Bmax_y, 0.27,A += (Bmax_y - Bmin_y)*t*5);
		fprintf(fp, "G1 X%f Y%f Z%f F1200\n", Bmin_x + 2 * i * 1 + 1, Bmax_y, 0.27);
		fprintf(fp, "G1 X%f Y%f Z%f F1200 A%f\n", Bmin_x + 2 * i * 1 + 1, Bmin_y, 0.27,A += (Bmax_y - Bmin_y)*t*5);
	}
	//fprintf(fp, ";LAYER:1\n");
	for (int i = 0; Bmin_y + 2 * i * 1 + 1 < Bmax_y; i++)  //这是第二层，二层以后都是0.2
	{
		fprintf(fp, "G1 X%f Y%f Z%f F1200\n", Bmin_x, Bmin_y + 2 * i * 1, 0.47);
		fprintf(fp, "G1 X%f Y%f Z%f F1200 A%f\n", Bmax_x, Bmin_y + 2 * i * 1, 0.47,A += (Bmax_y - Bmin_y)*t*5);
		fprintf(fp, "G1 X%f Y%f Z%f F1200\n", Bmax_x, Bmin_y + 2 * i * 1 + 1, 0.47);
		fprintf(fp, "G1 X%f Y%f Z%f F1200 A%f\n", Bmin_x, Bmin_y + 2 * i * 1 + 1, 0.47,A += (Bmax_y - Bmin_y)*t*5);
	}



	for (int i = 0; i < model.size() -2; i++) //i代表层数，要加几层打实在的，界限就减几，再改高度   
	{
		//fprintf(fp, ";LAYER:%d\n", i + 2);//加了几层底，这里就加几
		fprintf(fp, "M73 P%d\n", (i*100) / layers );//打印进度百分比
		...
		{
			//打印最内层
			fprintf(fp, "G1 X%.3f Y%.3f Z%f F2000\n", shell1[i][j][0].x, shell1[i][j][0].y, 0.67 + i * z);//3层高度加i*z，i从0开始

			fprintf(fp, "G1 X%.3f Y%.3f Z%f F1200 A%.5f\n", shell1[i][j][1].x, shell1[i][j][1].y, 0.67 + i * z, A += distance(shell1[i][j][0], shell1[i][j][1]) * t);

			for (int k = 2; k < shell1[i][j].size(); k++)

			{
				fprintf(fp, "G1 X%.3f Y%.3f Z%f F1200 A%.5f\n", shell1[i][j][k].x, shell1[i][j][k].y, 0.67 + i * z, A += distance(shell1[i][j][k - 1], shell1[i][j][k]) * t);
			...
			for (int k = 0; k < shell[i][j].size(); k++)
			{
				fprintf(fp, "G1 X%.3f Y%.3f Z%f F2000\n", shell[i][j][k][0].x, shell[i][j][k][0].y, 0.67 + i * z);//3层高度加i*z，i从0开始

				fprintf(fp, "G1 X%.3f Y%.3f Z%f F1200 A%.5f\n", shell[i][j][k][1].x, shell[i][j][k][1].y, 0.67 + i * z, A += distance(shell[i][j][k][0], shell[i][j][k][1]) * t);

				for (int m = 2; m < shell[i][j][k].size(); m++)

				{
					fprintf(fp, "G1 X%.3f Y%.3f Z%f F1200 A%.5f\n", shell[i][j][k][m].x, shell[i][j][k][m].y, 0.67 + i * z, A += distance(shell[i][j][k][m - 1], shell[i][j][k][m]) * t);
		...
		for (int j = 0; j < modfill[i].size(); j++)
		{
			for (int k = 1; k < modfill[i][j].size(); k += 2)
			{
				fprintf(fp, "G1 X%.3f Y%.3f Z%f F2000\n", modfill[i][j][k - 1].x, modfill[i][j][k - 1].y, 0.67 + i * z);
				fprintf(fp, "G1 X%.3f Y%.3f Z%f F1200 A%.5f\n", modfill[i][j][k].x, modfill[i][j][k].y, 0.67 + i * z,A += distance(modfill[i][j][k - 1], modfill[i][j][k]) * t);
			}
		}
...
void lines() //画线的函数
{
	...
	glBegin(GL_LINES);
	for (int i = 1; i < shell1.size(); i++)
	{
		for (int j = 0; j < shell1[i].size(); j++)
		{
			for(int k = 1;k<shell1[i][j].size();k++)
			{
				glVertex3f(shell1[i][j][k - 1].x, shell1[i][j][k - 1].y, i * 0.2); glVertex3f(shell1[i][j][k].x, shell1[i][j][k].y, i * 0.2);
			}
			glVertex3f(shell1[i][j][0].x, shell1[i][j][0].y, i * 0.2); glVertex3f(shell1[i][j][shell1[i][j].size() - 1].x, shell1[i][j][shell1[i][j].size() - 1].y, i * 0.2);
		}
	...
	glBegin(GL_LINES);
	for (int i = 1; i < modfill.size(); i += 19)
	{
		for (int j = 0; j < modfill[i].size(); j ++)
		{
			for (int k = 1; k < modfill[i][j].size(); k+=2)
			{
				glVertex3f(modfill[i][j][k - 1].x, modfill[i][j][k - 1].y, i * 0.2); glVertex3f(modfill[i][j][k].x, modfill[i][j][k].y, i * 0.2);
			}
