...
void writegcode_makerbot(float z)//还是用cura的格式然后用x3g转换吧
{
	...
	//fprintf(fp, ";LAYER:0\n");
	for (int i = 0; Bmin_x + 2 * i * 1.2 + 1.2 < Bmax_x; i++)  //第1、2层打实，第一层高度是0.27
	{
		fprintf(fp, "G1 X%f Y%f Z%f F1200\n", Bmin_x + 2 * i * 1.2, Bmin_y, 1);
		fprintf(fp, "G1 X%f Y%f Z%f F1200 A%f\n", Bmin_x + 2 * i * 1.2, Bmax_y, 1,A += (Bmax_y - Bmin_y)*t*5);
		fprintf(fp, "G1 X%f Y%f Z%f F1200\n", Bmin_x + 2 * i * 1.2 + 1.2, Bmax_y, 1);
		fprintf(fp, "G1 X%f Y%f Z%f F1200 A%f\n", Bmin_x + 2 * i * 1.2 + 1.2, Bmin_y, 1,A += (Bmax_y - Bmin_y)*t*5);
	}
	//fprintf(fp, ";LAYER:1\n");
	for (int i = 0; Bmin_y + 2 * i * 1.2 + 1.2 < Bmax_y; i++)  //这是第二层，二层以后都是0.2
	{
		fprintf(fp, "G1 X%f Y%f Z%f F1200\n", Bmin_x, Bmin_y + 2 * i * 1.2, 2);
		fprintf(fp, "G1 X%f Y%f Z%f F1200 A%f\n", Bmax_x, Bmin_y + 2 * i * 1.2, 2,A += (Bmax_y - Bmin_y)*t*5);
		fprintf(fp, "G1 X%f Y%f Z%f F1200\n", Bmax_x, Bmin_y + 2 * i * 1.2 + 1.2, 2);
		fprintf(fp, "G1 X%f Y%f Z%f F1200 A%f\n", Bmin_x, Bmin_y + 2 * i * 1.2 + 1.2, 2,A += (Bmax_y - Bmin_y)*t*5);
	}
	...
	for (int i = 0; i < model.size() -2; i++) //i代表层数，要加几层打实在的，界限就减几，再改高度   
	{
		...
		{
			//打印最内层
			fprintf(fp, "G1 X%.3f Y%.3f Z%f F2000\n", shell1[i][j][0].x, shell1[i][j][0].y, 2.2 + i * z);//3层高度加i*z，i从0开始

			fprintf(fp, "G1 X%.3f Y%.3f Z%f F1200 A%.5f\n", shell1[i][j][1].x, shell1[i][j][1].y, 2.2 + i * z, A += distance(shell1[i][j][0], shell1[i][j][1]) * t);

			for (int k = 2; k < shell1[i][j].size(); k++)

			{
				fprintf(fp, "G1 X%.3f Y%.3f Z%f F1200 A%.5f\n", shell1[i][j][k].x, shell1[i][j][k].y, 2.2 + i * z, A += distance(shell1[i][j][k - 1], shell1[i][j][k]) * t);
			...
			for (int k = 0; k < shell[i][j].size(); k++)
			{
				fprintf(fp, "G1 X%.3f Y%.3f Z%f F2000\n", shell[i][j][k][0].x, shell[i][j][k][0].y, 2.2 + i * z);//3层高度加i*z，i从0开始

				fprintf(fp, "G1 X%.3f Y%.3f Z%f F1200 A%.5f\n", shell[i][j][k][1].x, shell[i][j][k][1].y, 2.2 + i * z, A += distance(shell[i][j][k][0], shell[i][j][k][1]) * t);

				for (int m = 2; m < shell[i][j][k].size(); m++)

				{
					fprintf(fp, "G1 X%.3f Y%.3f Z%f F1200 A%.5f\n", shell[i][j][k][m].x, shell[i][j][k][m].y, 2.2 + i * z, A += distance(shell[i][j][k][m - 1], shell[i][j][k][m]) * t);
		...
		for (int j = 0; j < modfill[i].size(); j++)
		{
			for (int k = 1; k < modfill[i][j].size(); k += 2)
			{
				fprintf(fp, "G1 X%.3f Y%.3f Z%f F2000\n", modfill[i][j][k - 1].x, modfill[i][j][k - 1].y, 2.2 + i * z);
				fprintf(fp, "G1 X%.3f Y%.3f Z%f F1200 A%.5f\n", modfill[i][j][k].x, modfill[i][j][k].y, 2.2 + i * z,A += distance(modfill[i][j][k - 1], modfill[i][j][k]) * t);
			}
		}
