...
void writegcode_ultimaker3(float z) //cura
{
	...
	for (int i = 0; i < model.size() - 4; i++) //i代表层数，要加几层打实在的，界限就减几，再改高度   
	{
			...
			//轮廓线
		{
			...
			fprintf(fp, "G0 F2000 X%.3f Y%.3f Z%.1f\n", shell[i][j][0].x, shell[i][j][0].y, 0.3 + i * z);//3层高度加i*z，i从0开始

			fprintf(fp, "G1 F1200 X%.3f Y%.3f E%.5f\n", shell[i][j][1].x, shell[i][j][1].y, E += distance(shell[i][j][0], shell[i][j][1]) * t);

			for (int k = 2; k < shell[i][j].size(); k++)

			{
				fprintf(fp, "G1 X%.3f Y%.3f E%.5f\n", shell[i][j][k].x, shell[i][j][k].y, E += distance(shell[i][j][k - 1], shell[i][j][k]) * t);
